...
def morph_triangle(img1, img2, img, t1, t2, t, alpha) :
    ...
    h, w = min(r[1]+r[3], img.shape[0]) - r[1], min(r[0]+r[2], img.shape[1]) - r[0]
    mask, imgRect = mask[:h, :w], imgRect[:h, :w]
    img[r[1]:r[1]+h, r[0]:r[0]+w] = img[r[1]:r[1]+h, r[0]:r[0]+w] * ( 1 - mask ) + (imgRect * mask)
...
def generate_morph_sequence(duration,frame_rate,img1,img2,points1,points2,tri_list,size,output):
    ...
    for j in range(0, num_images):
        ...
        res.save(output+f'_{j}.jpg')

    # p.stdin.close()
    # p.wait()


def generate_morph_average(duration,frame_rate,img1,img2,points1,points2,tri_list,size,output):

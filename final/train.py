...
if __name__ == '__main__':
    ...
    if params.method in ['baseline']:
        if task in ["fsl", "cdfsl-single"]:
            datamgr = miniImageNet_few_shot.SimpleDataManager(
                image_size, batch_size=16)
            base_loaders.append(datamgr.get_data_loader(aug=params.train_aug))
            val_loaders.append(None)
        else:
            miniImageNet_datamgr = miniImageNet_few_shot.SimpleDataManager(
                image_size, batch_size=16)
            cifar100_datamgr = cifar100_few_shot.SimpleDataManager(
                image_size, batch_size=16)

            base_loaders.append(
                miniImageNet_datamgr.get_data_loader(aug=params.train_aug))
            val_loaders.append(None)
            base_loaders.append(
                cifar100_datamgr.get_data_loader(aug=params.train_aug))
            val_loaders.append(None)
        ...
        if task in ["fsl", "cdfsl-single"]:
            datamgr = miniImageNet_few_shot.SetDataManager(
                image_size, n_query=n_query, mode="train",  **train_few_shot_params)
            ...
            mini_ImageNet_datamgr = miniImageNet_few_shot.SetDataManager(
                image_size, n_query=n_query, mode="train",  **train_few_shot_params)
            mini_ImageNet_val_datamgr = miniImageNet_few_shot.SetDataManager(
                image_size, n_query=n_query, mode="val",  **test_few_shot_params)
            cifar100_datamgr = cifar100_few_shot.SetDataManager(
                image_size, n_query=n_query, mode="train",  **train_few_shot_params)
            cifar100_val_datamgr = cifar100_few_shot.SetDataManager(
                image_size, n_query=n_query, mode="val",  **test_few_shot_params)

            base_loaders.append(
                mini_ImageNet_datamgr.get_data_loader(aug=params.train_aug))
            val_loaders.append(
                mini_ImageNet_val_datamgr.get_data_loader(aug=False))
            base_loaders.append(
                cifar100_datamgr.get_data_loader(aug=params.train_aug))
    ...
    params.checkpoint_dir = '%s/ckpt/%s/%s_%s' % (
        save_dir, task_path, params.model, params.method)
    if params.train_aug:
    ...
    if task in ["fsl", "cdfsl-single"]:
        model = train(base_loaders[0], val_loaders[0], model,
                      optimization, start_epoch, stop_epoch, params)
    else:
        stop_epoch = stop_epoch // 2
        for i in range(2):
            print('now source domain ', i, ":")
            model = train(base_loaders[i], val_loaders[i], model,
                          optimization, start_epoch, stop_epoch, params)

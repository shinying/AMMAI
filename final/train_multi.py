...
from methods_multi.baselinetrain import BaselineTrain
from methods_multi.protonet import ProtoNet
...
def multi_train(base_loaders, val_loader, model, optimization, start_epoch, stop_epoch, params):
    if optimization == 'Adam':
        optimizer = torch.optim.Adam(model.parameters())
    else:
       raise ValueError('Unknown optimization, please define by yourself')
    ...
    for epoch in range(start_epoch,stop_epoch):
        model.train()
        model.train_loop(epoch, base_loaders,  optimizer)
...
if __name__=='__main__':
    ...
    if task in ["fsl", "cdfsl-single"]:
        ...
        model = multi_train(base_loaders, val_loaders[0], model, optimization, start_epoch, stop_epoch, params)

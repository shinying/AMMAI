...
class Classifier(nn.Module):
    ...
    def init(self, tensor):
        size = self.fc.weight.size()
        if size != tensor.size():
            raise ValueError(f'The input tensor should be size {size}.')
        self.fc.weight = nn.Parameter(tensor)
...
def meta_test(novel_loader, n_query=15, task='fsl', freeze_backbone=True, n_way=5, n_support=5):
    ...
    for ti, (x, y) in enumerate(novel_loader):
        ...
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' % (
            configs.save_dir, task_path, params.model, params.method)
        if params.train_aug:
            checkpoint_dir += '_aug'
        ...
        y_a_i = Variable(torch.from_numpy(
            np.repeat(range(n_way), n_support))).cuda()

        x_b_i = x_var[:, n_support:, :, :, :].contiguous().view(
            n_way * n_query,   *x.size()[2:])  # query set
        x_a_i = x_var[:, :n_support, :, :, :].contiguous().view(
            n_way * n_support, *x.size()[2:])  # support set
        ...
        classifier_opt = torch.optim.AdamW(classifier.parameters(), lr=3e-4, weight_decay=1e-4)
        # clas_scheduler = torch.optim.lr_scheduler.StepLR(classifier_opt, 30, 0.5)
        ...
        if not freeze_backbone:
            ...
            # delta_scheduler = torch.optim.lr_scheduler.StepLR(delta_opt, 30, 0.5)

        pretrained_model.cuda()

        # Feat: Initialize classifier with support set
        # pretrained_model.eval()
        # with torch.no_grad():
        #     output = pretrained_model(x_a_i).cpu()
        # prototypes = output.view(n_way, n_support, -1).mean(dim=1)
        # prototypes /= torch.sqrt(torch.sum(prototypes**2, dim=-1, keepdim=True))
        # classifier.init(prototypes)
        ...
        # with torch.no_grad():
            # protos = pretrained_model(x_a_i.cuda())
        # protos = protos.view(n_way, n_support, -1).mean(dim=1)
        # scores = torch.stack([-torch.pow(feat-protos, 2).sum(dim=1) for feat in output])
...
if __name__ == '__main__':
    ...
    ##################################################################
    image_size = 224
    iter_num = 600 # for fast experiments
    ...
    if task == 'fsl':
        ...
        datamgr = miniImageNet_few_shot.SetDataManager(
            image_size, n_eposide=iter_num, n_query=15, mode="test", **few_shot_params)
    ...
    for idx, novel_loader in enumerate(novel_loaders):
        ...
        meta_test(novel_loader, n_query=15, task=task,

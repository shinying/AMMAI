...
def meta_test(novel_loader, n_query=15, task='fsl', finetune=True, n_pseudo=100, n_way=5, n_support=5):
    ...
    for ti, (x, y) in enumerate(novel_loader):
        ...
        y_a_i = Variable(torch.from_numpy(
            np.repeat(range(n_way), n_support))).cuda()    # (25,)
        ...
        if finetune:
            ...
            pseudo_q_genrator = PseudoQeuryGenerator(
                n_way, n_support,  n_pseudo)
            delta_opt = torch.optim.AdamW(
                filter(lambda p: p.requires_grad, pretrained_model.parameters()),
                lr=1e-4, weight_decay=1e-4)
            
            # If use predictor, uncomment the following code
            # predictor = nn.Sequential(
            #     BatchNorm1d(512),
            #     nn.Linear(512, 512)).cuda()
            # predictor_opt = torch.optim.AdamW(predictor.parameters(), lr=3e-4, weight_decay=1e-5)
            ...
            for epoch in range(finetune_epoch):
                # SimSiam
                # Shuffle inputs
                idx = list(range(n_support))
                idxs = random.sample(idx, n_support)
                while sum(idx[i] == idxs[i] for i in range(n_support)):
                    idxs = random.sample(idx, n_support)
                idxs = [i*5+idxs[j] for i in range(n_way) for j in range(n_support)]

                x1, x2 = x_a_i, x_a_i[idxs]
                z1, z2 = pretrained_model.forward(x1), pretrained_model(x2)

                # Uncomment for predictor
                # p1, p2 = predictor(z1), predictor(z2)

                sim_loss = distance(z1, z2) / 2 + distance(z2, z1) / 2

                # generate pseudo query images
                psedo_query_set, _ = pseudo_q_genrator.generate(x_a_i)
                psedo_query_set = psedo_query_set.cuda().view(
                    n_way, fine_tune_n_query,  *x_a_i.size()[1:])

                x = torch.cat((z_support, psedo_query_set), dim=1)

                cls_loss = pretrained_model.set_forward_loss(x)
...
if __name__ == '__main__':
    ...
    for idx, novel_loader in enumerate(novel_loaders):
        ...
        meta_test(novel_loader, n_query=15, task=task,
                  finetune=finetune, n_pseudo=n_pseudo, **few_shot_params)
